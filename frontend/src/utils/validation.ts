...
export const validateRegisterNumber = (regNo: string): ValidationResult => {
    ...
    // yearCode (4-6) and serialNo (9-12) are just checked for numeric validity by the regex above.

    // 2. Validate College Code
    if (!COLLEGE_CODES.includes(collegeCode)) {
        return { isValid: false, error: `Invalid College Code: ${collegeCode}` };
    }

    // 3. Validate Year Code (Simple numeric check is already done by regex, 
    // but semantically any 2 digit year is "valid" in a broad sense. 
    // If specific ranges were needed we'd check here, e.g. 10-99).
    // Assuming any 2 digits are fine as per requirements provided.
    ...
    // 5. Validate Serial Number
    // Requirement: 0xx-9xx. This basically means 000-999.
    // The regex \d{3} covers this. 
    // However, usually serial numbers start from 001, but 0xx implies 000 is possible?
    // "Digits 10-12 (Serial Number):0xx-9xx" -> This just means any 3 digit number.
    // So no extra check needed beyond it being digits.
